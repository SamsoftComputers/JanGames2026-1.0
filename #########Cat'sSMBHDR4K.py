...
class MusicGenerator:
    ...
    def generate_level_music(self, world: int, stage: int) -> Optional[pygame.mixer.Sound]:
        """
        Generate music for a specific level - SMB1 1:1 accurate
        
        SMB1 Theme Assignment:
        - X-4: Castle theme (all worlds)
        - 1-2, 4-2: Underground theme
        - 2-2, 7-2: Underwater theme (actually 2-2 and 7-2 in original)
        - All others: Overworld theme
        """
        try:
            import numpy as np
            
            # Castle theme for all stage 4 levels
            if stage == 4:
                return self._generate_castle_theme(world)
            
            # Underground theme (1-2, 4-2)
            if (world == 1 and stage == 2) or (world == 4 and stage == 2):
                return self._generate_underground_theme(world)
            
            # Underwater theme (2-2, 7-2 in SMB1)
            if (world == 2 and stage == 2) or (world == 7 and stage == 2):
                return self._generate_underwater_theme()
            
            # Overworld theme for everything else (1-1, 1-3, 2-1, 2-3, 3-1, etc.)
...
class Game:
    ...
    def _get_level_structures(self, level: Level) -> List[dict]:
        """
        SMB1 1:1 ACCURATE LEVEL STRUCTURES
        All 32 levels with authentic Nintendo layouts
        Theme types: OVERWORLD, UNDERGROUND, UNDERWATER, CASTLE
        """
        structures = []
        ...
        low_y = ground_y - 3 * TILE_SIZE     # Low blocks

        w, s = level.world, level.stage

        # ==================== WORLD 1 ====================
        if w == 1 and s == 1:  # 1-1: OVERWORLD - The iconic first level
            ...
            structures.append({'type': 'qblock', 'x': 101, 'y': block_y, 'contains': 'star'})
            ...
            structures.append({'type': 'brick', 'x': 131, 'y': block_y})
            structures.append({'type': 'brick_row', 'x': 168, 'y': block_y, 'width': 3})
            
        elif w == 1 and s == 2:  # 1-2: UNDERGROUND
            structures.append({'type': 'brick_row', 'x': 5, 'y': high_y, 'width': 20})
            structures.append({'type': 'qblock', 'x': 12, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 25, 'y': block_y, 'width': 10})
            structures.append({'type': 'brick_row', 'x': 40, 'y': high_y, 'width': 15})
            structures.append({'type': 'qblock', 'x': 52, 'y': high_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 60, 'y': block_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 64, 'y': block_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 75, 'y': high_y, 'width': 12})
            structures.append({'type': 'brick_row', 'x': 95, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 97, 'y': block_y, 'contains': 'star'})
            
        elif w == 1 and s == 3:  # 1-3: ATHLETIC (treetops)
            structures.append({'type': 'brick_row', 'x': 12, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 14, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 25, 'y': high_y, 'width': 8})
            structures.append({'type': 'brick_row', 'x': 40, 'y': block_y, 'width': 5})
            structures.append({'type': 'brick_row', 'x': 55, 'y': high_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 60, 'y': high_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 75, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 77, 'y': block_y, 'contains': 'star'})
            structures.append({'type': 'brick_row', 'x': 95, 'y': high_y, 'width': 8})
            
        elif w == 1 and s == 4:  # 1-4: CASTLE
            structures.append({'type': 'brick_row', 'x': 15, 'y': block_y, 'width': 4})
            structures.append({'type': 'qblock', 'x': 17, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': high_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 55, 'y': block_y, 'width': 5})
            structures.append({'type': 'brick_row', 'x': 80, 'y': high_y, 'width': 8})

        # ==================== WORLD 2 ====================
        elif w == 2 and s == 1:  # 2-1: OVERWORLD
            structures.append({'type': 'qblock', 'x': 18, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 25, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 27, 'y': block_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 48, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 65, 'y': block_y, 'width': 7})
            structures.append({'type': 'qblock', 'x': 68, 'y': block_y, 'contains': 'coin'})
            structures.append({'type': 'qblock', 'x': 110, 'y': block_y, 'contains': 'star'})
            structures.append({'type': 'brick_row', 'x': 130, 'y': block_y, 'width': 5})
            
        elif w == 2 and s == 2:  # 2-2: UNDERWATER
            structures.append({'type': 'brick_row', 'x': 15, 'y': high_y, 'width': 10})
            structures.append({'type': 'brick_row', 'x': 35, 'y': block_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 38, 'y': block_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 55, 'y': high_y, 'width': 12})
            structures.append({'type': 'brick_row', 'x': 80, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 83, 'y': block_y, 'contains': 'mushroom'})
            
        elif w == 2 and s == 3:  # 2-3: ATHLETIC (bridges)
            structures.append({'type': 'brick_row', 'x': 15, 'y': block_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 18, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': high_y, 'width': 10})
            structures.append({'type': 'brick_row', 'x': 60, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 62, 'y': block_y, 'contains': 'star'})
            structures.append({'type': 'brick_row', 'x': 85, 'y': high_y, 'width': 8})
            
        elif w == 2 and s == 4:  # 2-4: CASTLE
            structures.append({'type': 'brick_row', 'x': 20, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 22, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': high_y, 'width': 7})
            structures.append({'type': 'brick_row', 'x': 70, 'y': block_y, 'width': 6})

        # ==================== WORLD 3 ====================
        elif w == 3 and s == 1:  # 3-1: OVERWORLD (night feel)
            structures.append({'type': 'brick_row', 'x': 15, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 17, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 38, 'y': high_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 60, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 62, 'y': block_y, 'contains': 'fireflower'})
            structures.append({'type': 'qblock', 'x': 110, 'y': block_y, 'contains': 'star'})
            
        elif w == 3 and s == 2:  # 3-2: OVERWORLD
            structures.append({'type': 'qblock', 'x': 20, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 30, 'y': block_y, 'width': 7})
            structures.append({'type': 'brick_row', 'x': 50, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 53, 'y': high_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 75, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 77, 'y': block_y, 'contains': 'fireflower'})
            
        elif w == 3 and s == 3:  # 3-3: ATHLETIC
            structures.append({'type': 'brick_row', 'x': 15, 'y': high_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 20, 'y': high_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 40, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 65, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 68, 'y': high_y, 'contains': 'star'})
            
        elif w == 3 and s == 4:  # 3-4: CASTLE
            structures.append({'type': 'brick_row', 'x': 18, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 20, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 40, 'y': high_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 65, 'y': block_y, 'width': 5})

        # ==================== WORLD 4 ====================
        elif w == 4 and s == 1:  # 4-1: OVERWORLD
            structures.append({'type': 'qblock', 'x': 20, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 28, 'y': block_y, 'width': 7})
            structures.append({'type': 'qblock', 'x': 31, 'y': block_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 50, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 54, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 75, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 100, 'y': block_y, 'contains': 'star'})
            
        elif w == 4 and s == 2:  # 4-2: UNDERGROUND
            structures.append({'type': 'brick_row', 'x': 10, 'y': high_y, 'width': 15})
            structures.append({'type': 'qblock', 'x': 18, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': block_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 40, 'y': block_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 55, 'y': high_y, 'width': 12})
            structures.append({'type': 'qblock', 'x': 62, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 80, 'y': block_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 105, 'y': block_y, 'contains': 'star'})
            
        elif w == 4 and s == 3:  # 4-3: ATHLETIC
            structures.append({'type': 'brick_row', 'x': 15, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 18, 'y': high_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 40, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 60, 'y': high_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 65, 'y': high_y, 'contains': 'star'})
            
        elif w == 4 and s == 4:  # 4-4: CASTLE (maze)
            structures.append({'type': 'brick_row', 'x': 15, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 17, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': high_y, 'width': 8})
            structures.append({'type': 'brick_row', 'x': 60, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 85, 'y': high_y, 'width': 5})

        # ==================== WORLD 5 ====================
        elif w == 5 and s == 1:  # 5-1: OVERWORLD
            structures.append({'type': 'brick_row', 'x': 18, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 20, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 40, 'y': high_y, 'width': 7})
            structures.append({'type': 'qblock', 'x': 43, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 65, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 90, 'y': block_y, 'contains': 'star'})
            
        elif w == 5 and s == 2:  # 5-2: OVERWORLD
            structures.append({'type': 'qblock', 'x': 22, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': block_y, 'width': 8})
            structures.append({'type': 'brick_row', 'x': 55, 'y': high_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 57, 'y': high_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 80, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 82, 'y': block_y, 'contains': 'fireflower'})
            
        elif w == 5 and s == 3:  # 5-3: ATHLETIC
            structures.append({'type': 'brick_row', 'x': 15, 'y': high_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 20, 'y': high_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 70, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 74, 'y': high_y, 'contains': 'star'})
            
        elif w == 5 and s == 4:  # 5-4: CASTLE
            structures.append({'type': 'brick_row', 'x': 20, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 22, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': high_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 70, 'y': block_y, 'width': 5})

        # ==================== WORLD 6 ====================
        elif w == 6 and s == 1:  # 6-1: OVERWORLD
            structures.append({'type': 'qblock', 'x': 22, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 30, 'y': block_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 33, 'y': block_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 55, 'y': high_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 57, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'qblock', 'x': 105, 'y': block_y, 'contains': 'star'})
            
        elif w == 6 and s == 2:  # 6-2: OVERWORLD
            structures.append({'type': 'brick_row', 'x': 20, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 22, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 48, 'y': high_y, 'contains': 'coin'})
            structures.append({'type': 'brick_row', 'x': 70, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 72, 'y': block_y, 'contains': 'fireflower'})
            
        elif w == 6 and s == 3:  # 6-3: ATHLETIC
            structures.append({'type': 'brick_row', 'x': 18, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 22, 'y': high_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 70, 'y': high_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 75, 'y': high_y, 'contains': 'star'})
            
        elif w == 6 and s == 4:  # 6-4: CASTLE
            structures.append({'type': 'brick_row', 'x': 18, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 20, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 42, 'y': high_y, 'width': 7})
            structures.append({'type': 'brick_row', 'x': 68, 'y': block_y, 'width': 5})

        # ==================== WORLD 7 ====================
        elif w == 7 and s == 1:  # 7-1: OVERWORLD
            structures.append({'type': 'brick_row', 'x': 20, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 23, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 48, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 70, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 95, 'y': block_y, 'contains': 'star'})
            structures.append({'type': 'qblock', 'x': 125, 'y': high_y, 'contains': '1up'})
            
        elif w == 7 and s == 2:  # 7-2: UNDERWATER
            structures.append({'type': 'brick_row', 'x': 20, 'y': high_y, 'width': 12})
            structures.append({'type': 'brick_row', 'x': 45, 'y': block_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 48, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 70, 'y': high_y, 'width': 10})
            structures.append({'type': 'brick_row', 'x': 95, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 97, 'y': block_y, 'contains': 'coin'})
            
        elif w == 7 and s == 3:  # 7-3: ATHLETIC
            structures.append({'type': 'brick_row', 'x': 18, 'y': high_y, 'width': 10})
            structures.append({'type': 'qblock', 'x': 23, 'y': high_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 48, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 72, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 76, 'y': high_y, 'contains': 'star'})
            
        elif w == 7 and s == 4:  # 7-4: CASTLE
            structures.append({'type': 'brick_row', 'x': 18, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 20, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 40, 'y': high_y, 'width': 8})
            structures.append({'type': 'brick_row', 'x': 65, 'y': block_y, 'width': 6})
            structures.append({'type': 'brick_row', 'x': 90, 'y': high_y, 'width': 5})

        # ==================== WORLD 8 (FINAL) ====================
        elif w == 8 and s == 1:  # 8-1: OVERWORLD (difficult)
            structures.append({'type': 'qblock', 'x': 25, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 35, 'y': block_y, 'width': 5})
            structures.append({'type': 'brick_row', 'x': 55, 'y': high_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 57, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 80, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 105, 'y': block_y, 'contains': 'star'})
            
        elif w == 8 and s == 2:  # 8-2: OVERWORLD (more enemies)
            structures.append({'type': 'brick_row', 'x': 22, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 24, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 50, 'y': high_y, 'width': 7})
            structures.append({'type': 'qblock', 'x': 53, 'y': high_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 75, 'y': block_y, 'width': 5})
            
        elif w == 8 and s == 3:  # 8-3: OVERWORLD (bullet bills)
            structures.append({'type': 'qblock', 'x': 28, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 40, 'y': block_y, 'width': 5})
            structures.append({'type': 'brick_row', 'x': 65, 'y': high_y, 'width': 8})
            structures.append({'type': 'qblock', 'x': 68, 'y': high_y, 'contains': 'star'})
            structures.append({'type': 'brick_row', 'x': 95, 'y': block_y, 'width': 6})
            
        elif w == 8 and s == 4:  # 8-4: FINAL CASTLE (Bowser!)
            structures.append({'type': 'brick_row', 'x': 20, 'y': block_y, 'width': 6})
            structures.append({'type': 'qblock', 'x': 22, 'y': block_y, 'contains': 'mushroom'})
            structures.append({'type': 'brick_row', 'x': 45, 'y': high_y, 'width': 8})
            structures.append({'type': 'brick_row', 'x': 70, 'y': block_y, 'width': 5})
            structures.append({'type': 'qblock', 'x': 72, 'y': block_y, 'contains': 'fireflower'})
            structures.append({'type': 'brick_row', 'x': 95, 'y': high_y, 'width': 6})

        # Add end staircase for non-castle levels
        if s != 4:
            for step in range(8):
    ...
    def _get_level_pipes(self, level: Level) -> List[dict]:
        ...
        Underground/Castle levels have fewer pipes, Athletic have none
        """
        w, s = level.world, level.stage

        # 1-1: Classic pipe layout
        if w == 1 and s == 1:
            return [
                {'x': 28, 'height': 2}, {'x': 38, 'height': 3},
                {'x': 46, 'height': 4}, {'x': 57, 'height': 4},
                {'x': 163, 'height': 2}, {'x': 179, 'height': 2},
            ]
        
        # Underground levels (1-2, 4-2) - minimal pipes
        if s == 2 and w in [1, 4]:
            return [{'x': 45, 'height': 2}, {'x': 90, 'height': 2}]
        
        # Underwater levels (2-2, 7-2) - no pipes
        if s == 2 and w in [2, 7]:
            return []
        
        # Castle levels - no pipes
        if s == 4:
            return []
        
        # Athletic levels (X-3) - minimal pipes
        if s == 3:
            return [{'x': 50, 'height': 2}, {'x': 100, 'height': 2}]
        
        # Overworld levels (X-1, X-2 for others) - standard pipe layout
        base_pipes = [
            {'x': 28, 'height': 2}, {'x': 38, 'height': 3},
            {'x': 57, 'height': 4},
        ]
        ...
        if w >= 2:
            base_pipes.append({'x': 95, 'height': 3})
        if w >= 4:
            base_pipes.append({'x': 125, 'height': 2})
        if w >= 6:
            base_pipes.append({'x': 155, 'height': 3})
    ...
    def _get_level_enemies(self, level: Level) -> List[dict]:
        ...
        Each level has authentic enemy types and positions
        """
        w, s = level.world, level.stage
        
        # ==================== WORLD 1 ====================
        if w == 1 and s == 1:  # 1-1: Goombas and one Koopa
            return [
                {'x': 22, 'type': 'goomba'}, {'x': 40, 'type': 'goomba'},
                {'x': 51, 'type': 'goomba'}, {'x': 52, 'type': 'goomba'},
                {'x': 80, 'type': 'goomba'}, {'x': 82, 'type': 'goomba'},
                {'x': 97, 'type': 'koopa'},
                {'x': 114, 'type': 'goomba'}, {'x': 115, 'type': 'goomba'},
                {'x': 124, 'type': 'goomba'}, {'x': 125, 'type': 'goomba'},
                {'x': 128, 'type': 'goomba'}, {'x': 129, 'type': 'goomba'},
                {'x': 174, 'type': 'goomba'}, {'x': 175, 'type': 'goomba'},
            ]
        elif w == 1 and s == 2:  # 1-2: Underground Goombas
            return [
                {'x': 25, 'type': 'goomba'}, {'x': 35, 'type': 'goomba'},
                {'x': 50, 'type': 'goomba'}, {'x': 65, 'type': 'goomba'},
                {'x': 80, 'type': 'goomba'}, {'x': 95, 'type': 'koopa'},
            ]
        elif w == 1 and s == 3:  # 1-3: Koopas on platforms
            return [
                {'x': 30, 'type': 'koopa'}, {'x': 55, 'type': 'koopa'},
                {'x': 80, 'type': 'goomba'}, {'x': 105, 'type': 'koopa'},
            ]
        elif w == 1 and s == 4:  # 1-4: Castle
            return [
                {'x': 40, 'type': 'goomba'}, {'x': 70, 'type': 'goomba'},
            ]
            
        # ==================== WORLD 2 ====================
        elif w == 2 and s == 1:
            return [
                {'x': 25, 'type': 'goomba'}, {'x': 26, 'type': 'goomba'},
                {'x': 45, 'type': 'koopa'}, {'x': 70, 'type': 'goomba'},
                {'x': 95, 'type': 'goomba'}, {'x': 96, 'type': 'goomba'},
                {'x': 120, 'type': 'koopa'}, {'x': 150, 'type': 'goomba'},
            ]
        elif w == 2 and s == 2:  # Underwater - no standard enemies
            return []
        elif w == 2 and s == 3:
            return [
                {'x': 30, 'type': 'koopa'}, {'x': 55, 'type': 'koopa'},
            ...
            return [
                {'x': 35, 'type': 'goomba'}, {'x': 60, 'type': 'goomba'},
                {'x': 85, 'type': 'goomba'},
            ]
        ...
        # ==================== WORLD 3 ====================
            ...
            return [
                {'x': 22, 'type': 'goomba'}, {'x': 23, 'type': 'goomba'},
                {'x': 45, 'type': 'koopa'}, {'x': 70, 'type': 'goomba'},
                {'x': 71, 'type': 'goomba'}, {'x': 100, 'type': 'koopa'},
                {'x': 130, 'type': 'goomba'}, {'x': 131, 'type': 'goomba'},
            ]
        elif w == 3 and s == 2:
            return [
                {'x': 28, 'type': 'goomba'}, {'x': 55, 'type': 'koopa'},
                {'x': 85, 'type': 'goomba'}, {'x': 86, 'type': 'goomba'},
                {'x': 115, 'type': 'koopa'},
            ]
        elif w == 3 and s == 3:
            return [
                {'x': 35, 'type': 'koopa'}, {'x': 60, 'type': 'koopa'},
                {'x': 85, 'type': 'goomba'}, {'x': 110, 'type': 'koopa'},
            ]
        elif w == 3 and s == 4:
            return [
                {'x': 40, 'type': 'goomba'}, {'x': 65, 'type': 'goomba'},
                {'x': 90, 'type': 'goomba'},
            ]
        ...
        # ==================== WORLD 4 ====================
            ...
            return [
                {'x': 25, 'type': 'goomba'}, {'x': 26, 'type': 'goomba'},
                {'x': 50, 'type': 'koopa'}, {'x': 75, 'type': 'goomba'},
                {'x': 100, 'type': 'koopa'}, {'x': 125, 'type': 'goomba'},
                {'x': 126, 'type': 'goomba'}, {'x': 150, 'type': 'koopa'},
            ]
        elif w == 4 and s == 2:  # Underground
            return [
                {'x': 30, 'type': 'goomba'}, {'x': 50, 'type': 'goomba'},
                {'x': 75, 'type': 'koopa'}, {'x': 100, 'type': 'goomba'},
            ]
        elif w == 4 and s == 3:
            return [
                {'x': 35, 'type': 'koopa'}, {'x': 60, 'type': 'koopa'},
                {'x': 85, 'type': 'koopa'}, {'x': 110, 'type': 'goomba'},
            ]
        elif w == 4 and s == 4:
            return [
                {'x': 35, 'type': 'goomba'}, {'x': 60, 'type': 'goomba'},
                {'x': 85, 'type': 'goomba'}, {'x': 110, 'type': 'goomba'},
            ]
            
        # ==================== WORLD 5 ====================
        elif w == 5 and s == 1:
            return [
                {'x': 25, 'type': 'koopa'}, {'x': 50, 'type': 'goomba'},
                {'x': 51, 'type': 'goomba'}, {'x': 80, 'type': 'koopa'},
                {'x': 110, 'type': 'goomba'}, {'x': 111, 'type': 'goomba'},
                {'x': 140, 'type': 'koopa'},
            ]
        elif w == 5 and s == 2:
            return [
                {'x': 30, 'type': 'goomba'}, {'x': 31, 'type': 'goomba'},
                {'x': 60, 'type': 'koopa'}, {'x': 90, 'type': 'goomba'},
                {'x': 120, 'type': 'koopa'},
            ]
        elif w == 5 and s == 3:
            return [
                {'x': 35, 'type': 'koopa'}, {'x': 65, 'type': 'koopa'},
                {'x': 95, 'type': 'koopa'},
            ]
        elif w == 5 and s == 4:
            return [
        ...
        # ==================== WORLD 6 ====================
            ...
            return [
                {'x': 25, 'type': 'koopa'}, {'x': 50, 'type': 'goomba'},
                {'x': 51, 'type': 'goomba'}, {'x': 52, 'type': 'goomba'},
                {'x': 85, 'type': 'koopa'}, {'x': 115, 'type': 'goomba'},
                {'x': 116, 'type': 'goomba'}, {'x': 145, 'type': 'koopa'},
            ]
        elif w == 6 and s == 2:
            return [
                {'x': 30, 'type': 'koopa'}, {'x': 60, 'type': 'goomba'},
                {'x': 61, 'type': 'goomba'}, {'x': 95, 'type': 'koopa'},
                {'x': 125, 'type': 'goomba'},
            ]
        elif w == 6 and s == 3:
            return [
                {'x': 40, 'type': 'koopa'}, {'x': 70, 'type': 'koopa'},
                {'x': 100, 'type': 'koopa'},
            ]
        elif w == 6 and s == 4:
            return [
                {'x': 35, 'type': 'goomba'}, {'x': 65, 'type': 'goomba'},
                {'x': 95, 'type': 'goomba'}, {'x': 125, 'type': 'goomba'},
            ]
            
        # ==================== WORLD 7 ====================
        elif w == 7 and s == 1:
            return [
                {'x': 25, 'type': 'koopa'}, {'x': 50, 'type': 'koopa'},
                {'x': 80, 'type': 'goomba'}, {'x': 81, 'type': 'goomba'},
                {'x': 110, 'type': 'koopa'}, {'x': 140, 'type': 'goomba'},
                {'x': 141, 'type': 'goomba'},
            ]
        elif w == 7 and s == 2:  # Underwater
            return []
        elif w == 7 and s == 3:
            return [
                {'x': 40, 'type': 'koopa'}, {'x': 70, 'type': 'koopa'},
            ...
            return [
                {'x': 35, 'type': 'goomba'}, {'x': 60, 'type': 'goomba'},
                {'x': 90, 'type': 'goomba'}, {'x': 120, 'type': 'goomba'},
            ]
            
        # ==================== WORLD 8 (FINAL) ====================
        elif w == 8 and s == 1:
            return [
                {'x': 25, 'type': 'koopa'}, {'x': 45, 'type': 'goomba'},
                {'x': 46, 'type': 'goomba'}, {'x': 47, 'type': 'goomba'},
                {'x': 75, 'type': 'koopa'}, {'x': 100, 'type': 'goomba'},
                {'x': 101, 'type': 'goomba'}, {'x': 130, 'type': 'koopa'},
            ]
        elif w == 8 and s == 2:
            return [
                {'x': 30, 'type': 'koopa'}, {'x': 55, 'type': 'goomba'},
                {'x': 56, 'type': 'goomba'}, {'x': 85, 'type': 'koopa'},
                {'x': 115, 'type': 'goomba'}, {'x': 116, 'type': 'goomba'},
            ]
        elif w == 8 and s == 3:
            return [
                {'x': 35, 'type': 'koopa'}, {'x': 65, 'type': 'koopa'},
        ...
        elif w == 8 and s == 4:  # Final castle
            return [
                {'x': 40, 'type': 'goomba'}, {'x': 70, 'type': 'goomba'},
                {'x': 100, 'type': 'goomba'}, {'x': 130, 'type': 'goomba'},
            ]
        
        # Fallback - should not reach here
        return [
            {'x': 30, 'type': 'goomba'}, {'x': 60, 'type': 'goomba'},
            {'x': 90, 'type': 'koopa'},
        ]
