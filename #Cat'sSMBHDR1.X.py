...
class Game:
    ...
    def _update(self):
        ...
        # Handle flagpole sequence (Mario slides down, then level complete)
        if self.flagpole_touched:
            self.level_complete_timer += 1
            # Mario slides down the pole
            if self.player.rect.bottom < self.flag.ground_y - TILE_SIZE:
                self.player.rect.y += 4
                self.player.vel_x = 0
                self.player.vel_y = 0
            elif self.level_complete_timer > 90:  # Wait a bit after landing
                # Store time bonus for COURSE CLEAR screen
                self.time_bonus = self.level_timer * 50  # 50 points per time unit
                self.course_clear_timer = 0
