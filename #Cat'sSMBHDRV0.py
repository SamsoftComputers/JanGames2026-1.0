#!/usr/bin/env python3
"""
CAT'S SUPER MARIO BROS v8
FIXED: Pits work properly + NES accurate Mario sprite
...
# =========================
# NES ACCURATE MARIO SPRITE
# Based on exact NES CHR ROM data
# Small Mario is 16x16 pixels
# =========================
def mario_spr(right=True):
    """
    NES-accurate Small Mario sprite
    Pixel-perfect recreation from original ROM
    """
    # 16x16 pixel art data (0=transparent, R=red, S=skin, B=brown, W=white, K=black)
    pixels = [
        "....RRRRR.......",  # Row 0: Hat top
        "...RRRRRRRR.....",  # Row 1: Hat
        "...BBBSSKB......",  # Row 2: Hair/forehead
        "..BSBSSKSSSK....",  # Row 3: Hair/face
        "..BSBBSSKSSS....",  # Row 4: Hair/face
        "..BBSSSSKBB.....",  # Row 5: Face
        "....SSSSSS......",  # Row 6: Face
        "...BBRRBBB......",  # Row 7: Shirt
        "..BBBRRBBRBB....",  # Row 8: Shirt
        ".SSBRRRRRRBS....",  # Row 9: Arms/shirt
        ".SSRRRRRRRRS....",  # Row 10: Arms/shirt
        "..RRRRRRRRRR....",  # Row 11: Shirt
        "..RRRBRRBRR.....",  # Row 12: Overalls
        "..BBB..BBB......",  # Row 13: Legs
        ".BBBB..BBBB.....",  # Row 14: Shoes
        "BBBBB..BBBBB....",  # Row 15: Shoes
    ]
    ...
    # Create at NES resolution (16x16) first for bit-accuracy
    nes_surf = pygame.Surface((16, 16), pygame.SRCALPHA)
    
    for y, row in enumerate(pixels):
        for x, c in enumerate(row):
            if c in colors and colors[c]:
                # Draw each pixel at exact NES coordinates (bit-accurate)
                pygame.draw.rect(nes_surf, colors[c], (x, y, 1, 1))
    
    # Scale up to TILE size (24x24) using smooth scaling for accuracy
...
def pipe_spr(h):
    """
    NES-accurate pipe sprite
    Pipes are 2 tiles wide (48px) in NES SMB1
    Bit-accurate pixel art recreation
    """
    # NES pipes are exactly 2 tiles wide
    pw = TILE * 2  # 48px - consistent width
    ph = TILE * h
    s = pygame.Surface((pw, ph), pygame.SRCALPHA)
    
    # Lip (top rim) - 8px tall, extends 4px on each side
    lip_h = 8
    # Outer border (dark green)
    pygame.draw.rect(s, DGN, (0, 0, pw, lip_h))
    # Inner fill (green)
    pygame.draw.rect(s, GRN, (2, 2, pw - 4, lip_h - 4))
    # Highlight (light green) - left side
    pygame.draw.rect(s, LGN, (4, 2, 4, lip_h - 4))
    
    # Body (narrower than lip, centered)
    body_x = 4  # 4px indent on each side
    body_w = pw - 8  # 40px wide body
    body_start = lip_h
    ...
    # Highlight (light green) - left side vertical stripe
...
# Gaps now specified as (start_x, end_x) - exclusive range
# =========================
LEVEL_1_1 = {
    ...
    # PITS: (start, end) - ground removed from start to end-1
    'pits': [(69, 71), (86, 89)],
}
...
class Game:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("Cat's SMB v8 - Fixed Pits + NES Mario")
        ...
        self.load()
        print("=" * 45)
        print(" CAT'S SUPER MARIO BROS v8")
        print(" FIXED: Pits + NES-accurate Mario sprite")
        print("=" * 45)
